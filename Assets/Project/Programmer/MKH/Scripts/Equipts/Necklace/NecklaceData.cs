...
namespace MKH
{
    ...
    public class NecklaceData : Item_Equipment
    {
        ...
        {
            List<Dictionary<string, object>> data = CSVReader.Read("EquipCSV - Necklace");
            ...
            if (ItemType.Necklace == Type)
            {
                ...
                // 주스텟
                createitem.mEffect.Mana = Random.Range((int)data[rate]["최소"], (int)data[rate]["마나"]);
                // 설명
                createitem.Name = (string)data[rate]["이름"];
                createitem.Description = $"마나 + {createitem.mEffect.Mana.ToString()}";
                ...
                for (int i = 0; i < rate; i++)  // Noraml : 0회 반복, Magic : 1회 반복, Rare : 2회 반복
                {
                    ...
                    switch (select)
                    {
                        ...
                        case 7:
                            ...
                            createitem.Description += $"\n스피드 + {(createitem.mEffect.Speed * 100f).ToString()}%";
                            break;
                    }
